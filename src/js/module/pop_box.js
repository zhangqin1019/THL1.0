...
define(["jquery"],function($){
	$.fn.createModal = function(options,callback) {
		...
		return this.each(function() {
			...
			$(modalContent).css({
				...
				"width": opts.width,
				"height": opts.height,
				"left": ($(window.parent).width() - parseInt(opts.width))/2,
				"top": ($(window.parent).height() - parseInt(opts.height))/2,
        ...
        if(opts.type == "popbox"){
			...
			$(modalContainer).append(modalBg).append(modalContent);//确定modalContainer的位置	
		}
        //弹页面
		else if(opts.type == "iframe"){
			$(modalIframe).attr('src',opts.iframeUrl);
			var page = $(modalContent).append(modalIframe);
            $(modalContainer).append(modalBg).append(page);//确定modalContainer的位置
		}
			// 父页面中添加弹窗
			$('body',window.parent.document).prepend(modalContainer);	
			...
			var writeModal = function(_$this){
				...
				$('body',window.parent.document).find('.modal-close').click(function(){
						$(modalContent).empty();
						$(modalContainer).remove();
				})
	...
	$.fn.createModal.defaults = {
		...
		resizable: true,
		bgClose: false,
		type:"popbox",//popbox,iframe
		// iframeUrl:"http://localhost:3000/page/detail_query.html",
